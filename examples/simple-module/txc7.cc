	...
	void Txc7::handleMessage(cMessage *msg)
	{
	    if (msg == event) {
	        ...
	        tictocMsg = nullptr;
	               ...
	               // "Lose" the message with 0.1 probability: -- ba ehtemal 0.1 msg daryafti drop mikonim
	               if (uniform(0, 1) < 0.1) {
	                   EV << "\"Losing\" message\n";
	                   delete msg;
	               }
	                   ...
	                   // we'll get a different delay every time. -- dar inja ba gereftan parameter delay az .ned ya .ini va pas az delay random payam dobare ersal mikonad.
...
//nokte qabele ahmiat in ast k omnet++ dar har bar run j motefavet nemidahad balke yek j sabet midahad.
